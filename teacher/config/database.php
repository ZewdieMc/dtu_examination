...
class Database
{
    ...
    public function get_student_result($exam_id)
    {;
        $query = "
SELECT S.STUDENT_ID,S.FIRST_NAME,S.LAST_NAME, RE.EXAM_ID,T.TOTAL AS SCORE, Q.TOTAL AS WEIGHT, C.COURSE_NAME AS COURSE_NAME FROM TBL_RESULT AS RE 
...
LEFT JOIN(SELECT STUDENT_ID, SUM(MARKS) AS TOTAL FROM TBL_RESULT WHERE USER_ANSWER=RIGHT_ANSWER AND EXAM_ID = $exam_id GROUP BY STUDENT_ID)T 
ON T.STUDENT_ID = RE.STUDENT_ID
INNER JOIN (SELECT SUM(MARKS) AS TOTAL FROM TBL_QUESTION WHERE EXAM_ID = $exam_id) Q WHERE RE.EXAM_ID = $exam_id GROUP BY RE.STUDENT_ID;
